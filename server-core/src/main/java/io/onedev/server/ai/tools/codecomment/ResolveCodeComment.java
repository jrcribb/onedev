...
public final class ResolveCodeComment implements TaskTool {

	@Override
	public ToolSpecification getSpecification() {
		return ToolSpecification.builder()
				.name("resolveCodeComment")
				.description("Mark a line-anchored code comment as resolved, optionally with a short note")
				.parameters(JsonObjectSchema.builder()
						.addIntegerProperty("commentId").description("Existing code comment id")
						.addStringProperty("note").description("Optional note explaining why it is resolved")
						.required("commentId")
						.build())
				.build();
	}
	...
	public ToolExecutionResult execute(Subject subject, JsonNode arguments) {
		...
		try {
			var commentInfo = CodeCommentHelper.changeStatus(subject, comment, true, note);
